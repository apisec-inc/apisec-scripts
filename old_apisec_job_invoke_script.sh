...
if [ "$SCAN_ALL_PROJECTS" = true ]; then
    
     projList=$(curl -s -X GET "$FX_HOST/api/v1/projects/myProjects?page=0&pageSize=20&sort=createdDate&sortType=DESC" -H "accept: */*"  --header "Authorization: Bearer "$token"" | jq -r '.data[]|.name')
     #echo "$projList"
     tprojCount=1
     for proj in ${projList}
       do
		  ...
		  echo "The request is $url"
                  ...
                  echo "ProjectName: $proj"
                  echo "Scanned project count No. $tprojCount"
                  ...
                  then
                         echo "RunId = " "$runId"
                         echo "Invalid runid"
                         echo $(curl -s --location --request POST "${FX_HOST}/api/v1/runs/project/${proj}?jobName=${JOB_NAME}&region=${REGION}&emailReport=${FX_EMAIL_REPORT}&reportType=${FX_REPORT_TYPE}${FX_SCRIPT}" --header "Authorization: Bearer "$token"" | jq -r '.["data"]|.id')
                         exit 1
                  fi
                               ...
                               if [ "$taskStatus" == "COMPLETED" ];then
                                        ...
                                        if [ "$FAIL_ON_HIGH_VULNS" = true ]; then
                                               ...
                                               for vul in ${severity}
                                                ...
                                                echo "Found $cVulCount Critical Severity Vulnerabilities in $proj project!!!"
                                                echo " "
                                                ...
                                                echo "Found $hVulCount High Severity Vulnerabilities in $proj project!!!"
                                                echo " "
                                                ...
                                                echo "Found $majVulCount Major Severity Vulnerabilities in $proj project!!!"
                                                echo " "
                                                ...
                                                echo "Found $medVulCount Medium Severity Vulnerabilities in $proj project!!!"
                                                echo " "
                                                ...
                                                echo "Found $minVulCount Minor Severity Vulnerabilities in $proj project!!!"
                                                echo " "
                                                ...
                                                echo "Found $lowVulCount Low Severity Vulnerabilities in $proj project!!!"
                                                echo " "
                                                ...
                                                echo "Found $triVulCount Trivial Severity Vulnerabilities in $projS projectS!!!"
                                                echo " "
                                                 ...
                                                 for vul in ${severity}
                                                     do
                                                         ...
                                                         if  [ "$vul" == "Critical"  ] || [ "$vul" == "High"  ] ; then
                                                                 echo "Failing script execution since we have found "$vul" severity vulnerability in $proj project!!!"
                                                                 exit 1
