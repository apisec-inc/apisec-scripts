...
do
   ...
   do
     ...
     runId=$(curl -s -k --location --request POST "https://${FX_HOSTNAME}/api/v1/runs/job/${jobID}?region=${REGION}&tags=&suites=&categories=ABAC_Level1,%20InvalidAuth,%20InvalidAuthEmpty,%20InvalidAuthSQL,%20Unsecured,%20Incremental_Ids,%20PII,%20ADoS,%20Ratelimit_Authenticated,%20Ratelimit_Unauthenticated,%20RBAC,%20Ssrf_External_Url,%20Cors_Config,%20Info_Leak_Ip_Header,%20TLS_Headers,%20NoSQL_Injection,%20Ssl_Certificate_Issues,%20Server_Properties_Leak_In_Headers,%20Linux_Command_Injection,%20Log4j_Injection,%20Reflected_Get_Injection,%20Reflected_Headers,%20Reflected_Post_Injection,%20SQL_Injection_Timebound,%20NoSQL_Injection_Filter,%20SQL_Injection_Filter,%20XSS_Injection,%20Windows_Command_Injection,%20Insecure_Cookies,%20&emailReport=false&projectId=${project_id}&env=${envID}&endpoints=" --header "Authorization: Bearer "$token"" | jq -r '.["data"]|.id')
