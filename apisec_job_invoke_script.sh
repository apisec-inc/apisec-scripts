                             ...
                             if [ "$taskStatus" == "COMPLETED" ];then
                                    ...
                                    if [ "$FX_S3_REPORT_TYPE_FLAG" = true ]; then
                                          ...
                                          if [ $errorMsgFlag = false ]; then
                                               ...
                                               isProjStorageAccountNull=$( echo "$projData" | jq -r '.account')
                                               IsProjectStorageAccountInActive=$( echo "$projData" | jq -r '.inactive')
                                               #echo "Is Project StorageAccount Null/epmty: $isProjStorageAccountNull"
